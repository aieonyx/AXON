...
fn cmd_build(args: &[String]) {
    // Parse flags: axon build [--native] [--target <t>] [--profile <p>] [-o <out>] <file.axon>
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--target" => {
                i += 1;
                if i < args.len() { target_name = Some(args[i].clone()); i += 1; }
            }
            "--profile" | "-p" => {
                i += 1;
                if i < args.len() { profile_name = Some(args[i].clone()); i += 1; }
            }
            "--output" | "-o" => {
                i += 1;
                if i < args.len() { output_name = Some(args[i].clone()); i += 1; }
            }
    ...
    // Phase 8 pipeline: --profile flag activates sovereign compiler
    if profile_name.is_some() || output_name.is_some() {
        cmd_build_phase8(&file, profile_name.as_deref(), output_name.as_deref());
        return;
    }
...
fn cmd_build_phase8(file: &str, profile_str: Option<&str>, output: Option<&str>) {
    ...
    if !module.errors.is_empty() {
        for e in &module.errors {
            eprintln!("axon: HIR error: {}", e.msg);
        }
        std::process::exit(1);
    }

    // Profile enforcement — SEC3: violations are fatal
    ...
    let out_dir = Path::new(file).parent().unwrap_or(Path::new("."))
        .to_string_lossy().to_string();
    let bin_name = output.unwrap_or(&stem).to_string();
    let bin_path = if output.is_some() {
        bin_name.clone()
    } else {
        format!("{}/{}", out_dir, stem)
    };
    ...
    // Compile to object
    let obj_path = match ir_to_object(&ir, "/tmp") {
        Ok(p) => p,
        Err(e) => {
            eprintln!("axon: compile error: {}", e);
            std::process::exit(1);
        }
    };

    // Link to binary
    match object_to_binary(&obj_path, &bin_path) {
        Ok(()) => {
            println!("axon: binary ready: {}", bin_path);
            println!("axon: run: {}", bin_path);
        }
        Err(e) => {
            eprintln!("axon: link error: {}", e);
            std::process::exit(1);
        }
    }
}
