...
/// IAM-M2 sovereign tokenizer hash — locked July 2026.
/// Generated by: cargo run -p axon_data --bin train_tokenizer
/// Corpus: KJV + Stoics + 9,660 training pairs
/// Vocab: 8,000 BPE entries, 7,712 merge rules
/// This hash is immutable — changing it breaks checkpoint resume.
pub const IAM_SEED_TOKENIZER_HASH: &str = "efddfa3d92ea3b6128f5a558babebe43993a5387e6bbf4701529572e605b4297";
...
pub struct TrainingConfig {
    ...
    // ── Tokenizer lock ──────────────────────────────────────────────────────
    /// Sovereign tokenizer hash — must match .axvocab used to build corpus
    /// Set at IAM-M2, immutable for the lifetime of this training run
...
impl TrainingConfig {
    ...
    pub fn iam_seed() -> Self {
        Self {
            ...
            stage:           "seed".into(),
            tokenizer_hash:  IAM_SEED_TOKENIZER_HASH.into(),
        }
    }
    ...
    /// Verify a tokenizer hash matches this config's expected hash.
    pub fn tokenizer_hash_matches(&self, hash: &str) -> bool {
        self.tokenizer_hash.is_empty() || self.tokenizer_hash == hash
    }

    /// Get the expected tokenizer hash.
    pub fn expected_tokenizer_hash(&self) -> &str {
        &self.tokenizer_hash
    }
...
impl std::fmt::Display for ConfigError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Self::MissingTokenizerHash  => write!(f, "tokenizer_hash not set — run IAM-M2 first"),
        }
    }
}
